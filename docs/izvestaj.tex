...
\title{
    ...
    \normalsize Primena klasifikacije na problem selekcije algoritma (Rice, 1976)
}
...
tome da li se mogu izračunati iz same strukture stabla ili zahtevaju da neki algoritam
(AB, MCTS ili orakl) zaista bude pokrenut:
...
ab\_ranking\_correlation & X & X & X & 3 \\
...
\begin{itemize}[noitemsep]
    ...
    zahteva da se AB, MCTS ili orakl zaista pokrenu (\texttt{ab\_nodes\_visited},
    \texttt{ab\_ranking\_correlation}, \texttt{mcts\_vote\_margin}, \texttt{exact\_best\_value},
...
Oba modela slažu se da budžet (\texttt{node\_budget}, \texttt{budget\_coverage}) i strukturna
margina korena (\texttt{branch\_top2\_gap}) čine jezgro najvažnijih atributa, u skladu sa
...
\midrule
\texttt{ab\_correct}   & 0.825 & 0.696 \\
\texttt{mcts\_correct} & 0.849 & 0.769 \\
\bottomrule
\end{tabular}
\end{table}

Razlika između skupa A i skupa B je veća za \texttt{ab\_correct} nego za
...
AUC $= 0.5$ slučajnog modela. Atributi u skupu A (posebno \texttt{mcts\_vote\_margin}
i \texttt{ab\_ranking\_correlation}) dodaju realnu vrednost kod oba targeta,
...
delimično predvidive iz strukture, ali daleko od trivijalno. Medium kategorija je
...
heuristike nezavisni dobri prediktori. Jedini atribut vezan striktno za prvi nivo dubine koji
ostaje u top deset je \texttt{chance\_node\_count\_d1}.
...
(posebno \texttt{mcts\_vote\_margin}, \texttt{ab\_ranking\_correlation}) nose
stvarnu dodatnu vrednost, ali ne menjaju koji tip strukture je bitan.

\subsection{Ograničenja}

Stabla su sintetička i kontrolisanog oblika, realna stabla igara (šah, Catan) imaju bogatiju
strukturu i specifične evaluacione funkcije, pa generalizabilnost nalaza na stvarne
igre nije eksperimentalno proverena. Iscrpni minimax kao izvor istine nije primenjiv na igre
sa milijardama čvorova, pa bi na takvim igrama selekcija algoritma morala da se osloni na
aproksimativan algoritam. 
